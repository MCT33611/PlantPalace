$(document).ready(function () {
    // Example: Call loadDataTable with a specific category
    //var category = "exampleCategory";
    console.log('yes');
    loadDataTable();
});

function loadDataTable() {
    console.log('yes');

    dataTabel = $('#tblData').DataTable({
        "ajax": {
            url: '/admin/product/getall',
            error: function (xhr, error, thrown) {
                console.log("DataTables error:", error, thrown);
            }
        },
        "columns": [

            { data: 'name', "width": "20%" },
            { data: 'category.name', "width": "15%" },
            { data: 'listPrice', "width": "10%" },
            { data: 'price', "width": "10%" },
            { data: 'price50', "width": "10%" },
            { data: 'price100', "width": "10%" },
            {


                data: 'id',
                "render": function (data) {
                    return`<a href="Product/Upsert?id=${data}" class="btn btn-primary rounded me-1 text-nowrap">
                                <i class="bi bi-pencil-square"></i> Edit
                            </a>
                            <a onClick = Delete('Product/Delete/${data}') class="btn btn-danger rounded ms-1 text-nowrap">
                                <i class="bi bi-trash3-fill"></i> Delete
                            </a> `
                },
                            
                "width": "25%"
            },

        ]
    });
}

function Delete(url) {
    Swal.fire({
        title: 'Are you sure?',
        text: "You won't be able to revert this!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Yes, delete it!'
    }).then((result) => {
        if (result.isConfirmed) {
            $.ajax({
                url: url,
                type: 'DELETE',
                success: function (data) {
                    dataTabel.ajax.reload();
                    toastr.success(data.message)
                }
            })
        }
    })
}